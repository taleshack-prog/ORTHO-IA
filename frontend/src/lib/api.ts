...
export const patients = {
  list: (p?: { search?: string; page?: number }) => { const qs = new URLSearchParams(p as Record<string,string>).toString(); return req<{ total: number; patients: Patient[] }>(`/patients${qs ? `?${qs}` : ""}`); },
  get: (id: string) => req<Patient>(`/patients/${id}`),
  create: (d: Partial<Patient>) => req<Patient>("/patients", { method: "POST", body: JSON.stringify(d) }),
};
export const cases = {
  list: (p?: { patient_id?: string; page?: number }) => { const qs = new URLSearchParams(Object.fromEntries(Object.entries(p||{}).filter(([,v])=>v!==undefined)) as Record<string,string>).toString(); return req<{ total: number; cases: Case[] }>(`/cases${qs ? `?${qs}` : ""}`); },
  ...
  uploadImages: (caseId: string, files: File[]) => { const fd = new FormData(); files.forEach(f => fd.append("files", f)); const token = typeof window !== "undefined" ? localStorage.getItem("orto_token") : null; return fetch(`${API}/cases/${caseId}/images`, { method: "POST", headers: token ? { Authorization: `Bearer ${token}` } : {}, body: fd }).then(r => r.json()); },
};
